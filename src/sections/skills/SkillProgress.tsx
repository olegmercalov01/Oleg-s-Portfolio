...
export default function SkillProgress({ skills, activeCategory }: SkillProgressProps) {
  ...
  const skillLevels = {
    beginner: filteredSkills.filter(skill => skill.level < 70).length,
    intermediate: filteredSkills.filter(skill => skill.level >= 70 && skill.level < 89).length,
    advanced: filteredSkills.filter(skill => skill.level >= 89).length
  };
